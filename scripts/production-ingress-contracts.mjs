...
const checks = [
  ...
  ['ContactV1 terminal requires signed production authority', /^      ContactService__RuntimeActivation: "true"$/m.test(compose) && /^      ContactAuthority__Enabled: "true"$/m.test(compose) && /^      ContactAuthority__XPointNetworkGenesisPinHex: \$\{DEEP_XPOINT_GENESIS_PIN_HEX:\?set DEEP_XPOINT_GENESIS_PIN_HEX\}$/m.test(compose) && /^      ContactRouteClosure__Enabled: "true"$/m.test(compose) && /^      RequiredTerminals__Contact: "true"$/m.test(compose)],
  ['GroupControl is not falsely advertised before dynamic authority publication', /^      RequiredTerminals__GroupControl: "false"$/m.test(compose) && !/^      GroupControlService__RuntimeActivation: "true"$/m.test(compose)],
