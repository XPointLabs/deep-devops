...
const contacts = await Promise.all([41801, 41802, 41803].map(async port => {
  const response = await fetch(`http://127.0.0.1:${port}/api/network/contact`);
...
if (result.seeded !== 3) throw new Error('relay-bootstrap did not accept exactly three contacts');
process.stdout.write('Seeded three signed relay contacts.\n');
