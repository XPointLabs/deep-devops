...
try {
    ...
    if ($primary['MailboxClient__DevelopmentFixture__CoordinatorUrl'] -cne 'http://127.0.0.1:41801' -or
        $primary['MailboxClient__Enabled'] -cne 'true' -or
        $primary['MailboxClientAdapter__Enabled'] -cne 'true') {
        throw 'Host-only authority generation did not bind the single exact coordinator.'
    }
