...
- `xnode-1`, `xnode-2`, `xnode-3`: three-router release rehearsal profile used by `scripts/multi-node-rehearsal.ps1`; default rehearsal uses the real Xray-backed router image and records a unique `artifacts/rehearsals/multi-node/<run-id>/test-results/multi-node-topology.json`.
