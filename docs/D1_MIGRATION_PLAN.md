...
- Storage compatibility runtime now also performs real signature verification for signed `/storage/store`, `/storage/retrieve`, `/storage/get_expiries`, `/storage/expire_all`, `/storage/expire`, `/storage/delete`, `/storage/delete_all`, and `/storage/delete_before` requests when the pubkey is directly verifiable (`03...` or `05...` + `pubkey_ed25519`), and uses `sig_timestamp` for store signature verification when provided.
